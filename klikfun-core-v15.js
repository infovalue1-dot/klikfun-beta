...
async function loadRewardFile(ev){const f=ev.target.files&&ev.target.files[0];if(!f||rewardFixed)return;stopCamera();const img=new Image(),url=URL.createObjectURL(f);await new Promise((res,rej)=>{img.onload=res;img.onerror=rej;img.src=url});const c=$('camCanvas'),ratio=img.width/img.height;c.width=360;c.height=480;const sc=Math.max(c.width/img.width,c.height/img.height),dw=img.width*sc,dh=img.height*sc;c.getContext('2d').drawImage(img,(c.width-dw)/2,(c.height-dh)/2,dw,dh);URL.revokeObjectURL(url);rewardShot=true;rewardDownloaded=false;rewardDataUrl=c.toDataURL('image/jpeg',.9);$('camVideo').classList.add('hidden');$('camCanvas').classList.remove('hidden');$('rewardEdit').classList.remove('hidden');['camBtn','snapBtn','switchCameraBtn'].forEach(id=>$(id)?.classList.add('hidden'));await refreshRewardSubject();applyStylePreviewFinal()}
