...
function renderGroupDashboardFinal(d,host){showOnly('groupRound');kfGroupCode=d.code||kfGroupCode;const people=d.participants||[],allDone=people.length>0&&people.every(x=>x.completed);$('groupStatus').textContent=people.filter(x=>x.completed).length+'/'+people.length+' peserta selesai';const links=JSON.parse(localStorage.getItem('kf_group_links_'+kfGroupCode)||'[]');$('groupPeople').innerHTML=people.map(p=>{const l=links.find(x=>Number(x.slot)===Number(p.slot));return `<div class="metric"><span>Peserta ${p.slot}</span><b>${p.completed?'✓ selesai':'menunggu'}</b></div>${l&&!p.completed?`<button class="btn ghost" onclick="shareGroupParticipantFinal(${p.slot},'${l.token}')">Kirim link peserta ${p.slot}</button>`:''}`}).join('');if(allDone){const once='kf_group_shared_granted_'+kfGroupCode;if(localStorage.getItem(once)!=='1'){grantReward('group-shared');localStorage.setItem(once,'1')}localStorage.removeItem('kf_group_links_'+kfGroupCode);localStorage.removeItem('kf_group_admin_'+kfGroupCode);kfGroupAdmin=null;$('groupSharedRewardBtn').classList.toggle('hidden',!rewardEntitlement('group-shared'))}else $('groupSharedRewardBtn').classList.add('hidden');if(host){clearInterval(kfGroupPoll);kfGroupPoll=allDone?null:setInterval(refreshGroupFinal,8000)}}
...
async function refreshGroupFinal(){if(!kfGroupCode)return;const key=kfGroupAdmin||localStorage.getItem('kf_group_admin_'+kfGroupCode);if(!key)return;try{const r=await fetch(KF_API.group+'/'+kfGroupCode,{headers:{'X-Klikfun-Admin-Key':key}}),d=await r.json();if(r.ok)return renderGroupDashboardFinal(d,true);if(r.status===404||r.status===403){clearInterval(kfGroupPoll);kfGroupPoll=null;localStorage.removeItem('kf_group_links_'+kfGroupCode);localStorage.removeItem('kf_group_admin_'+kfGroupCode);$('groupStatus').textContent='Grup sudah berakhir.'}}catch(e){}}
