...
function renderGroupDashboardFinal(d,host){showOnly('groupRound');kfGroupCode=d.code||kfGroupCode;const people=d.participants||[],allDone=people.length>0&&people.every(x=>x.completed);$('groupStatus').textContent=people.filter(x=>x.completed).length+'/'+people.length+' peserta selesai';const links=JSON.parse(localStorage.getItem('kf_group_links_'+kfGroupCode)||'[]');$('groupPeople').innerHTML=people.map(p=>{const l=links.find(x=>Number(x.slot)===Number(p.slot));return `<div class="metric"><span>Peserta ${p.slot}</span><b>${p.completed?'✓ selesai':'menunggu'}</b></div>${l?`<button class="btn ghost" onclick="shareGroupParticipantFinal(${p.slot},'${l.token}')">Kirim link peserta ${p.slot}</button>`:''}`}).join('');if(allDone){const once='kf_group_shared_granted_'+kfGroupCode;if(localStorage.getItem(once)!=='1'){grantReward('group-shared');localStorage.setItem(once,'1')}$('groupSharedRewardBtn').classList.toggle('hidden',!rewardEntitlement('group-shared'))}else $('groupSharedRewardBtn').classList.add('hidden');if(host){clearInterval(kfGroupPoll);kfGroupPoll=allDone?null:setInterval(refreshGroupFinal,8000)}}
