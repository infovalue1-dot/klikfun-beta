...
function goHome(){stopCamera();if(typeof clearRewardMedia==='function')clearRewardMedia();showOnly('home');loadPublicCount()}
...
async function copyTextSafe(text){
  ...
  const area=document.createElement('textarea');area.value=value;area.setAttribute('readonly','');area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();area.setSelectionRange(0,value.length);let copied=false;try{copied=document.execCommand('copy')}catch(_){}area.remove();return copied
}
async function shareOrCopy(payload,fallbackText){
  if(navigator.share){try{await navigator.share(payload);return 'shared'}catch(e){if(e&&e.name==='AbortError')return 'cancelled'}}
  return await copyTextSafe(fallbackText)?'copied':'failed'
}
...
async function shareInviteFinal(){if(!kfActiveCode)return;const url=inviteUrlFinal(kfActiveCode),result=await shareOrCopy({title:'Klikfun',text:'Coba tebak jawaban aku di Klikfun',url},url);if(result==='shared')kfTrack('invite_shared',{channel:'native'});else if(result==='copied'){kfTrack('invite_copied');alert('Link disalin.')}else if(result==='failed')alert('Link belum dapat dibagikan. Coba tekan Salin link.')}
async function copyInviteFinal(){if(!kfActiveCode)return;const copied=await copyTextSafe(inviteUrlFinal(kfActiveCode));if(copied){kfTrack('invite_copied');alert('Link disalin.')}else alert('Link belum dapat disalin. Tekan lama pada link undangan untuk menyalinnya.')}
...
async function createGroupFinal(){const size=Math.max(2,Math.min(5,Number($('groupSize').value||2)));try{const assignments=uniqueAssignments(size),r=await fetch(KF_API.group,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({size,assignments,consent:true})}),d=await r.json();if(!r.ok)throw Error(d.error||'Gagal membuat grup');kfGroupCode=d.code;kfGroupAdmin=d.admin_key;sessionStorage.setItem('kf_group_admin_'+kfGroupCode,kfGroupAdmin);sessionStorage.setItem('kf_group_links_'+kfGroupCode,JSON.stringify(d.participants||[]));renderGroupDashboardFinal(d,true);kfTrack('group_started',{mode:String(size)});kfLogActivity('group','Membuat grup '+size+' peserta')}catch(e){alert(e.message||'Grup belum bisa dibuat.')}}
function groupParticipantUrlFinal(code,slot,token){const u=new URL(location.href);u.search='';u.hash='';u.searchParams.set('group',code);u.searchParams.set('slot',slot);u.hash=new URLSearchParams({token}).toString();return u.toString()}
function renderGroupDashboardFinal(d,host){showOnly('groupRound');kfGroupCode=d.code||kfGroupCode;const people=d.participants||[],allDone=people.length>0&&people.every(x=>x.completed);$('groupStatus').textContent=people.filter(x=>x.completed).length+'/'+people.length+' peserta selesai';const links=JSON.parse(sessionStorage.getItem('kf_group_links_'+kfGroupCode)||'[]');$('groupPeople').innerHTML=people.map(p=>{const l=links.find(x=>Number(x.slot)===Number(p.slot));return `<div class="metric"><span>Peserta ${p.slot}</span><b>${p.completed?'✓ selesai':'menunggu'}</b></div>${l&&!p.completed?`<button class="btn ghost" onclick="shareGroupParticipantFinal(${p.slot},'${l.token}')">Kirim link peserta ${p.slot}</button>`:''}`}).join('');if(allDone){const once='kf_group_shared_granted_'+kfGroupCode;if(localStorage.getItem(once)!=='1'){grantReward('group-shared');localStorage.setItem(once,'1')}sessionStorage.removeItem('kf_group_links_'+kfGroupCode);sessionStorage.removeItem('kf_group_admin_'+kfGroupCode);kfGroupAdmin=null;$('groupSharedRewardBtn').classList.toggle('hidden',!rewardEntitlement('group-shared'))}else $('groupSharedRewardBtn').classList.add('hidden');if(host){clearInterval(kfGroupPoll);kfGroupPoll=allDone?null:setInterval(refreshGroupFinal,8000)}}
async function shareGroupParticipantFinal(slot,token){const url=groupParticipantUrlFinal(kfGroupCode,slot,token),result=await shareOrCopy({title:'Klikfun Grup',text:'Masuk sebagai Peserta '+slot,url},url);if(result==='copied')alert('Link peserta '+slot+' disalin.');else if(result==='failed')alert('Link peserta belum dapat dibagikan.')}
async function refreshGroupFinal(){if(!kfGroupCode)return;const key=kfGroupAdmin||sessionStorage.getItem('kf_group_admin_'+kfGroupCode);if(!key)return;try{const r=await fetch(KF_API.group+'/'+kfGroupCode,{headers:{'X-Klikfun-Admin-Key':key}}),d=await r.json();if(r.ok)return renderGroupDashboardFinal(d,true);if(r.status===404||r.status===403){clearInterval(kfGroupPoll);kfGroupPoll=null;sessionStorage.removeItem('kf_group_links_'+kfGroupCode);sessionStorage.removeItem('kf_group_admin_'+kfGroupCode);$('groupStatus').textContent='Grup sudah berakhir.'}}catch(e){}}
function clearSensitiveGroupUrl(){const u=new URL(location.href);u.searchParams.delete('group');u.searchParams.delete('slot');u.searchParams.delete('token');u.hash='';history.replaceState(null,'',u.pathname+(u.searchParams.toString()?'?'+u.searchParams.toString():''))}
async function joinGroupFromLinkFinal(code,slot,token){clearSensitiveGroupUrl();try{const r=await fetch(KF_API.group+'/'+encodeURIComponent(code)+'/participant/'+slot,{headers:{'X-Klikfun-Participant-Token':token}}),d=await r.json();if(!r.ok)throw Error(d.error||'Undangan grup tidak berlaku');kfGroupCode=code;kfGroupParticipantToken=token;kfGroupSlot=slot;kfGroupQuestions=d.questions;kfGroupAnswers=[];kfGroupIndex=0;if(d.completed){showOnly('groupRound');$('groupStatus').textContent='Jawabanmu sudah tersimpan.';$('groupPeople').innerHTML='';return}renderGroupPlayFinal()}catch(e){goHome();alert(e.message||'Undangan grup tidak berlaku.')}}
...
async function shareJanjiFinal(){const j=window.kfCurrentJanji;if(!j)return;const u=new URL(location.href);u.search='';u.hash='';u.searchParams.set('janji',j.id);u.searchParams.set('ja',String(ACTIVITIES.indexOf(j.activity)));u.searchParams.set('jt',String(TIMES.indexOf(j.time)));const text=`JANJI Klikfun: ${j.activity} · ${j.time}`,value=text+'\n'+u.toString(),result=await shareOrCopy({title:'JANJI Klikfun',text,url:u.toString()},value);if(result==='copied')alert('JANJI dan link disalin.');else if(result==='failed')alert('JANJI belum dapat dibagikan.')}
...
const SUBJECT_STYLE_MAP={face:['natural','smooth','cinematic-beauty','elegant','charming'],multi_face:['natural','bright-face','cinematic-beauty'],food:['food-pop','vivid-scene','warm'],product:['product-clean','cinematic-scene','cool'],nature:['nature','vivid-scene','cinematic-scene'],vehicle:['cinematic-scene','vivid-scene','product-clean'],object:['product-clean','cinematic-scene','vivid-scene'],scene:['natural-scene','vivid-scene','cinematic-scene'],unknown:['natural-scene','cinematic-scene','natural']};
...
async function loadRewardFile(ev){
 const f=ev.target.files&&ev.target.files[0];if(!f||rewardFixed)return;
 if(!['image/jpeg','image/png','image/webp'].includes(String(f.type||'').toLowerCase())){alert('Gunakan foto JPG, PNG, atau WEBP.');ev.target.value='';return}
 if(f.size>8*1024*1024){alert('Ukuran foto maksimal 8 MB.');ev.target.value='';return}
 stopCamera();const img=new Image(),url=URL.createObjectURL(f);
 try{
  await new Promise((res,rej)=>{img.onload=res;img.onerror=()=>rej(Error('Foto tidak dapat dibaca.'));img.src=url});
  if(!img.width||!img.height)throw Error('Foto tidak dapat dibaca.');
  const c=$('camCanvas');c.width=360;c.height=480;const sc=Math.max(c.width/img.width,c.height/img.height),dw=img.width*sc,dh=img.height*sc;c.getContext('2d').drawImage(img,(c.width-dw)/2,(c.height-dh)/2,dw,dh);rewardShot=true;rewardDownloaded=false;rewardDataUrl=c.toDataURL('image/jpeg',.9);$('camVideo').classList.add('hidden');c.classList.remove('hidden');$('rewardEdit').classList.remove('hidden');['camBtn','snapBtn','switchCameraBtn'].forEach(id=>$(id)?.classList.add('hidden'));await refreshRewardSubject();applyStylePreviewFinal()
 }catch(e){alert(e.message||'Foto tidak dapat dibuka.');ev.target.value=''}finally{URL.revokeObjectURL(url)}
}
...
function rewardGrantStore(){try{const raw=JSON.parse(localStorage.getItem(KF_STORAGE.reward)||'null');if(raw?.version===2&&raw.grants&&typeof raw.grants==='object')return raw;if(raw?.source)return{version:2,grants:{[raw.source]:raw}}}catch(_){}return{version:2,grants:{}}}
function writeRewardGrantStore(store){localStorage.setItem(KF_STORAGE.reward,JSON.stringify(store))}
function grantReward(source){const store=rewardGrantStore(),ent={source,grantedAt:Date.now(),expiresAt:Date.now()+24*3600000,used:false,downloaded:false};store.grants[source]=ent;writeRewardGrantStore(store);return ent}
function rewardEntitlement(source){const e=rewardGrantStore().grants[source];return e&&!e.used&&Number(e.expiresAt)>Date.now()?e:null}
function updateRewardEntitlement(source,patch){const store=rewardGrantStore(),e=store.grants[source];if(!e)return;store.grants[source]={...e,...patch};writeRewardGrantStore(store)}
function clearRewardMedia(){rewardDataUrl=null;rewardShot=false;const c=$('camCanvas');if(c){c.style.filter='none';c.getContext('2d')?.clearRect(0,0,c.width,c.height)}const file=$('rewardFile');if(file)file.value=''}
function unlockRewardFinal(source){if(!rewardEntitlement(source)){alert('Reward Camera belum terbuka untuk aktivitas ini.');return}rewardUnlockSource=source;clearRewardMedia();rewardFixed=false;rewardDownloaded=false;$('rewardSource').textContent=source==='group-shared'?'Reward grup bersama · maksimal 5 wajah':'Reward pribadi · '+source;renderStyleOptionsFinal();renderAiThemesFinal();$('rewardEdit').classList.add('hidden');$('rewardFixed').classList.add('hidden');$('camVideo').classList.remove('hidden');$('camCanvas').classList.add('hidden');showOnly('rewardCamera')}
...
function finalizeRewardUI(){$('rewardEdit').classList.add('hidden');$('rewardFixed').classList.remove('hidden');updateRewardEntitlement(rewardUnlockSource,{used:true,fixedAt:Date.now()});stopCamera()}
function downloadRewardFinal(){if(!rewardFixed||!rewardDataUrl||rewardDownloaded)return;const a=document.createElement('a');a.href=rewardDataUrl;a.download='klikfun-reward-final.jpg';document.body.appendChild(a);a.click();a.remove();rewardDownloaded=true;updateRewardEntitlement(rewardUnlockSource,{downloaded:true,downloadedAt:Date.now()});clearRewardMedia();$('downloadBtn').disabled=true;$('downloadBtn').textContent='Sudah didownload · terkunci';kfTrack('reward_downloaded');kfLogActivity('reward','Reward Camera selesai')}
