...
function fakeAiTransform(){
  ...
  alert('Prototype: AI Transform sengaja tidak memanggil layanan berbayar. Pada runtime final hanya free quota yang boleh dipakai, tanpa auto-paid.')
}
...
function showMemberGateFinal(){showOnly('memberGate');const c=memberCapabilities();$('memberGate').innerHTML=`<h2>Member Klikfun</h2><div class="metric"><span>Passkey</span><b>${c.passkey?'didukung perangkat':'tidak didukung'}</b></div><div class="metric"><span>Recovery code</span><b>siap arsitektur</b></div><div class="metric"><span>Member ID</span><b>siap arsitektur</b></div><div class="metric"><span>Inbox JANJI</span><b>siap arsitektur</b></div><div class="notice warn">Registrasi tidak dipalsukan di browser. Fitur member aktif hanya setelah endpoint akun/passkey resmi tersedia.</div><button class="btn ghost" onclick="showOnly('welcome')">Balik</button>`}
...
function publishPortalRecruitment(){
 ...
 $('recruitmentSummary').innerHTML=`<b>${p.subtheme}</b><br>Kamu memakai <b>${load.hero.name}</b> (${load.hero.subrole}). Party ${p.party}: ${humans} manusia · ${bots} Bot Hero. Bot memprioritaskan fungsi party yang belum tersedia.`;
 ...
 $('recruitmentRoster').innerHTML=roster.map(x=>`<div class="inventory-item"><b>${x.name}</b><div class="small">${x.role} · ${x.isBot?'Bot Hero':'Pemain manusia'}</div></div>`).join('');
...
function showGameModeFinal(mode){window.KF_GAME_ENTRY='arena';$('gameModeTitle').textContent='Arena 5v5';showOnly('gameMode');gameInitRegistry();const fill=gameBotFill(4),bots=gameBotRoster(4);$('matchInfo').innerHTML=`Arena 5v5 · tidak ada farming EXP/level di match. Contoh minimum: 4 manusia + ${fill.bots} bot = 10 slot; bot ${Math.round(fill.bots/10*100)}% (maksimum 60%). ${bots.map(b=>`<span class="bot-pill">${b.name}</span>`).join('')}`;startGameDemo()}
...
function enterSelectedPortal(){
 ...
 $('portalMissionTitle').textContent=p.subtheme;$('portalMissionMeta').textContent=`${p.family} · ${p.env} · Party ${p.party} (${humans} manusia + ${bots} Bot Hero)`;
...
function portalActionStart(){
 ...
 portalActionLog(st.type==='loot'?'Tidak ada monster. Sinyal loot terdeteksi.':'Combat dimulai. Bot Hero bergerak otomatis membantu tim.');
...
function portalActionDamagePlayer(dmg,now){
 ...
 if(st.hp<=0){st.alive=false;st.respawnAt=now+5000;portalActionSetStatus('Hero tumbang · respawn 5 detik');portalActionLog('Hero tumbang. Bot Hero tetap bertarung.')}
}
...
function showGameFinal(){showOnly('game');const n=$('game').querySelector('.notice.ok');if(n)n.textContent=`Runtime perangkat: ${DEVICE_RUNTIME.backend} · tier ${DEVICE_RUNTIME.tier}. WebGPU → WebGL2/WebGL → Canvas/CPU. Map/monster Portal berasal dari database; server tidak merender frame dan tidak memakai AI generatif untuk gameplay.`}
